...
app.get('/forexCheck', (req, res) => {
  res.status(200).render('forexCheck', {
    title: 'Forex Check', fromValue: 0, toValue: 0, 
    defaultCurrency: [{fromCurrency: 'USD', toCurrency: 'INR'}],
    currencyList: [
...
app.post('/forexCheck', async (req, res) => {

  try {
    let body = await req.body;

    let { fromCurrency, fromValue, toCurrency } = body;

    if (fromValue === '') {
      res.status(200).render('forexCheck', {
        title: 'Forex Check', fromValue: 0, toValue: "Please Enter Value",
        defaultCurrency: [{fromCurrency, toCurrency}],
        currencyList: [
          { name: "Afghan Afghani", code: "AFA" },
          { name: "Albanian Lek", code: "ALL" },
          { name: "Algerian Dinar", code: "DZD" },
      ...
      res.status(200).render('forexCheck', {
        title: 'Forex Check', fromValue: 0, toValue: "Negative Values Not Allowed", 
        defaultCurrency: [{fromCurrency, toCurrency}],
        currencyList: [
          { name: "Afghan Afghani", code: "AFA" },
          { name: "Albanian Lek", code: "ALL" },
      ...
      if (fromCurrency === toCurrency) {
        res.status(200).render('forexCheck', {
          title: 'Forex Check', fromValue: 0, toValue: "Same Currencies Not Allowed", 
          defaultCurrency: [{fromCurrency, toCurrency}],
          currencyList: [
            { name: "Afghan Afghani", code: "AFA" },
            { name: "Albanian Lek", code: "ALL" },
        ...
        let response = await fetch(`https://v6.exchangerate-api.com/v6/${process.env.API_DASHBOARD}/latest/${fromCurrency}`)

        let jsData = await response.json();

        let conversion_rates = jsData.conversion_rates;

        for (curr in conversion_rates) {
          if (toCurrency === curr) {
            let toValue = fromValue * conversion_rates[curr]
            res.status(200).render('forexCheck', { title: 'Forex Check', fromValue, toValue,
            defaultCurrency: [{fromCurrency, toCurrency}], 
            currencyList: [
              {name: "Afghan Afghani", code: "AFA"},
              {name: "Albanian Lek", code: "ALL"},
...
app.get('/topcurrencies', async (req, res) => {
  ...
  const fetchApi = async () => {
    ...
    currencies.map(async (currency) => {
      ...
      let response = await fetch(`https://v6.exchangerate-api.com/v6/${process.env.API_DASHBOARD}/latest/${currency}`)

      let jsData = await response.json()

      let currencyValue = {
        base: jsData.base_code,
        INR: jsData.conversion_rates.INR,
        lastUpdate: jsData.time_last_update_utc,
        nextUpdate: jsData.time_next_update_utc
      }

      currencyValues = await currencyValues.concat(currencyValue)

    })

    return await currencyValues

  }

  await fetchApi()

  // res.status(200).render('topCurrencies', {
  //   title: 'Top Currencies', currencyValues: [
  //     {
  //       "base": "CAD",
  //       "INR": 60.8716,
  //       "lastUpdate": "Fri, 12 Apr 2024 00:00:01 +0000",
  //       "nextUpdate": "Sat, 13 Apr 2024 00:00:01 +0000",
  //       "img": `/flags/CAD.jpg`
  //     },
  //     {
  //       "base": "USD",
  ...
  setTimeout(()=>{
      res.status(200).render('topCurrencies', {title: 'Top Currencies', currencyValues})
  }, 2500)
